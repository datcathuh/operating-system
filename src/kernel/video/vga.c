...
struct vga_mode vga_mode_text_80x25 = {
	...
	.crtc = {0x5F, 0x4F, 0x50, 0x82, 0x55, 0x81, 0xBF, 0x1F, 0x00,
             0x4F, 0x0D, 0x0E, 0x00, 0x00, 0x00, 0xF0, 0x9C, 0x8E,
             0x8F, 0x28, 0x1F, 0x96, 0xB9, 0xA3, 0xFF},
	.gfx = {0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x0E, 0x0F, 0xFF},
	.attr = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x14, 0x07, 0x38, 0x39, 0x3A,
...
struct vga_mode vga_mode_320x200x256 = {
	...
	.gfx = {0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x05, 0x0F, 0xFF},
	.attr = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
...
static void vga_color_get(struct terminal * /*terminal*/,
                          enum terminal_color *fg, enum terminal_color *bg) {
	*fg = current_color & 0xf0;
	*bg = current_color >> 4;
}

static void vga_color_set(struct terminal * /*terminal*/,
                          enum terminal_color fg, enum terminal_color bg) {
	current_color = vga_color_entry(fg, bg);
}

static void vga_scroll(struct video_device *device) {
	for (uint32_t i = 0; i < ((uint32_t)device->resolution->height - 1) *
	                             (uint32_t)device->resolution->width * 2;
	     ++i) {
		device->buffer->memory[i] =
			device->buffer->memory[i + device->resolution->width * 2];
	}
	for (uint32_t i =
	         (device->resolution->height - 1) * device->resolution->width * 2;
	     i < device->resolution->height * device->resolution->width * 2;
	     i += 2) {
		device->buffer->memory[i] = ' ';
...
static void vga_clear(struct terminal *terminal) {
	struct video_device *device = terminal->data;
	for (uint32_t i = 0;
	     i < device->resolution->width * device->resolution->height * 2;
	     i += 2) {
		device->buffer->memory[i] = ' ';
		device->buffer->memory[i + 1] = current_color;
	}
...
static void vga_print_char(struct terminal *terminal, char c) {
	struct video_device *device = terminal->data;
	if (c == '\n') {
		cursor = ((cursor / 2) / device->resolution->width + 1) *
		         device->resolution->width * 2;
		if ((cursor / 2) / device->resolution->width >=
...
static inline void vga_write_indexed(uint16_t idx_port, uint16_t data_port,
...
static void vga_mode_set(struct vga_mode *mode) {
	...
	if (mode->flags == vga_mode_text) {
		...
		pokew(0x40, 0x4C,
		      mode->width * mode->height * 2); /* framebuffer size */
...
struct terminal _terminal_vga = {.size = vga_size,
...
static bool vga_device_initialize(struct video_device *) { return true; }

static bool vga_device_resolution_set(struct video_device *device,
...
struct video_device _vga_device = {.resolution = 0,
                                   .pci_device = 0,
                                   .buffer = &_vga_buffer,
...
struct video_device *vga_device() { return &_vga_device; }
