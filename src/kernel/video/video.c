...
/* Draw a single pixel at x,y. framebuffer is a pointer to the start of the
 * buffer. pitch = bytes per scanline. bytes_per_pixel = 4 or 2. color32 is
 * 0xRRGGBB (no alpha). */
static inline void video_set_pixel(void *framebuffer, int pitch,
                                   int bytes_per_pixel, int x, int y,
                                   uint32_t color32) {
	...
	if (bytes_per_pixel == 4) {
		...
		/* If your BGA expects BGRA or little-endian order, this still works on
		   little-endian x86: px in memory will be 0xBBGGRR00 but most systems
		   render 0xRRGGBB when reading 32-bit as 0x00RRGGBB; adapt if needed.
		 */
		*(uint32_t *)fb = px;
 ...
 * font must point to 256 * FONT_HEIGHT bytes; glyph c starts at
 * font[c*FONT_HEIGHT]. Foreground and background colors are 0xRRGGBB.
 */
void video_draw_char(void *framebuffer, int pitch, int bytes_per_pixel,
                     int screen_width, int screen_height, const uint8_t *font,
                     unsigned char c, int px, int py, uint32_t fg_color,
	...
	if (px + glyph_w <= 0 || py + glyph_h <= 0 || px >= screen_width ||
	    py >= screen_height)
		return;
			...
			for (int sy = 0; sy < scale; ++sy) {
				...
				for (int sx = 0; sx < scale; ++sx) {
					...
					if (x < 0 || x >= screen_width)
						continue;
					video_set_pixel(framebuffer, pitch, bytes_per_pixel, x, y,
					                color);
				}
			}
		}
...
void video_draw_string(struct video_buffer *buffer, const uint8_t *font, int px,
                       int py, const char *s, uint32_t fg_color,
                       uint32_t bg_color, int scale) {
	int pitch = buffer->resolution.width * buffer->resolution.bpp / 8;
	int x = px;
	int y = py;
	while (*s) {
		...
		if (*s == '\b') {
			x -= FONT_WIDTH;
			video_draw_char(buffer->memory, pitch, buffer->resolution.bpp / 8,
			                buffer->resolution.width, buffer->resolution.height,
			                font, ' ', x, y, fg_color, bg_color, scale);
			s++;
			continue;
		}
		video_draw_char(buffer->memory, pitch, buffer->resolution.bpp / 8,
		                buffer->resolution.width, buffer->resolution.height,
		                font, (unsigned char)*s, x, y, fg_color, bg_color,
		                scale);
		x += FONT_WIDTH * scale;
		s++;
...
void video_draw_line(struct video_buffer *buffer, int x0, int y0, int x1,
	...
	while (1) {
		...
		if (x0 < buffer->resolution.width && y0 < buffer->resolution.height) {
			video_set_pixel(buffer->memory, pitch, buffer->resolution.bpp / 8,
			                x0, y0, color);
		}
...
void video_draw_rect(struct video_buffer *buffer, int x, int y, int width,
...
void video_draw_rect_filled(struct video_buffer *buffer, int x, int y,
	...
	if (y < 0)
		y = 0;
	if (x_end > buffer->resolution.width)
		x_end = buffer->resolution.width;
	if (y_end > buffer->resolution.height)
		y_end = buffer->resolution.height;
	...
	for (int py = y; py < y_end; ++py) {
		for (int px = x; px < x_end; ++px) {
			video_set_pixel(buffer->memory, pitch, bytes_per_pixel, px, py,
			                color);
		}
	}
}
