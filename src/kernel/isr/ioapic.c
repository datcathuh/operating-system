...
static inline void ioapic_write(uint32_t ioapic_base, uint32_t reg, uint32_t val) {
...
void ioapic_setup(void) {
	...
	mem_page_map(ioapics[0].address, ioapics[0].address, MEM_PAGE_PRESENT|MEM_PAGE_WRITABLE|MEM_PAGE_NO_CACHE|MEM_PAGE_NX);
}

void ioapic_set_irq(uint32_t gsi, uint8_t vector, uint8_t lapic_id, uint64_t flags) {
	struct acpi_ioapic *ioapics;
	int ioapic_count = 0;
	acpi_ioapic_get(&ioapics, &ioapic_count);
    ...
    ioapic_write(ioapics[0].address, IOAPIC_REDTBL + pin * 2 + 1, (uint32_t)(entry >> 32));
}
