...
static void draw_hud(int score, int level, int next_type) {
    ...
    video_draw_string(vd, NULL, sx, sy, buf, TEXT_COLOR, BG_COLOR, 1);
    // snprintf(buf, sizeof(buf), "Level: %d", level);
    video_draw_string(vd, NULL, sx, sy+18, buf, TEXT_COLOR, BG_COLOR, 1);
    video_draw_string(vd, NULL, sx, sy+36, "Controls:", TEXT_COLOR, BG_COLOR, 1);
    video_draw_string(vd, NULL, sx, sy+54, "a: left  d: right", TEXT_COLOR, BG_COLOR, 1);
    video_draw_string(vd, NULL, sx, sy+72, "s: down  w: rot", TEXT_COLOR, BG_COLOR, 1);
    video_draw_string(vd, NULL, sx, sy+90, "space: hard drop", TEXT_COLOR, BG_COLOR, 1);
    video_draw_string(vd, NULL, sx, sy+108, "q: quit", TEXT_COLOR, BG_COLOR, 1);

    /* next piece preview */
    video_draw_string(vd, NULL, sx, sy+130, "Next:", TEXT_COLOR, BG_COLOR, 1);
        ...
        */
        ...
        if (lock_now) {
            ...
            if (piece_collides(&cur, board)) {
                ...
                video_draw_string(vd, NULL, cx, cy, go, TEXT_COLOR, BG_COLOR, 1);
                /* wait for 'q' */
                while (1) {
