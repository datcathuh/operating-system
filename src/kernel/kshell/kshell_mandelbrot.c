...
static void kshell_mandelbrot_cb(void) {
	...
	for (int py = 0; py < height; ++py) {
		for (int px = 0; px < width; ++px) {
			int32_t x0 =
				(int32_t)(((int64_t)(px - width / 2) * scale_numer) / width);
			int32_t y0 =
				(int32_t)(((int64_t)(py - height / 2) * scale_numer) / height);
			...
			while (1) {
				...
				int32_t ynew = (int32_t)((xy2 >> 15) +
				                         y0); /* 2*x*y -> (x*y<<1) which is
				                                 (x*y)>>15 after >>16 adjust */
				x = xtemp;
				y = ynew;
			...
			uint8_t color1 = (iteration == max_iteration)
			                     ? 0
			                     : (uint8_t)((iteration * 5) & 0xFF);
...
void kshell_mandelbrot_register() {
	struct kshell_command cmd = {.name = "mandelbrot",
