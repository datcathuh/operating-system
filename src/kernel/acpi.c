...
static uint8_t acpi_checksum(const uint8_t *addr, int len) {
    uint8_t sum = 0;
    for (int i = 0; i < len; ++i) sum = (uint8_t)(sum + addr[i]);
    return sum;
}

static int rsdp_validate(rsdp_descriptor_t *rsdp) {
    if (rsdp == NULL) return 0;
    for (int i = 0; i < 8; ++i) if (rsdp->signature[i] != ACPI_SIG_RSDP[i]) return 0;
    if (rsdp->revision == 0) {
        return acpi_checksum((uint8_t*)rsdp, 20) == 0;
    } else {
        if (rsdp->length < 36) return 0;
...
static rsdp_descriptor_t *find_rsdp(void) {
    ...
    for (uintptr_t p = start; p < end; p += 16) {
        ...
        if (candidate->signature[0] == 'R' && candidate->signature[1] == 'S' &&
            ...
            if (rsdp_validate(candidate)) return candidate;
        }
    }
    return NULL;
}
...
static void print_sig4(const char *sig) {
    for (int i = 0; i < 4; ++i) {
        char c = sig[i];
        if (c >= 32 && c < 127) serial_putc(c); else serial_putc('?');
    }
}

/* Find an SDT by signature by scanning RSDT or XSDT entries */
static uintptr_t acpi_find_table_from_rsdt(acpi_sdt_header_t *rsdt, const char *sig) {
    if (!rsdt) return 0;
    if (acpi_checksum((uint8_t*)rsdt, rsdt->length) != 0) return 0;
    ...
    for (uint32_t i = 0; i < entries; ++i) {
        ...
        if (hdr->signature[0] == sig[0] && hdr->signature[1] == sig[1] && hdr->signature[2] == sig[2] && hdr->signature[3] == sig[3])
            return (uintptr_t)hdr;
    }
    return 0;
}

static uintptr_t acpi_find_table_from_xsdt(acpi_sdt_header_t *xsdt, const char *sig) {
    if (!xsdt) return 0;
    if (acpi_checksum((uint8_t*)xsdt, xsdt->length) != 0) return 0;
        ...
        switch (h->type) {
            case 0: {
                if (ent + sizeof(madt_local_apic_t) <= end) {
                    ...
                    if (la->flags & 1) serial_puts(" (ENABLED)"); else serial_puts(" (DISABLED)");
                    serial_puts("\n");
                }
                break;
            }
...
void acpi_init(void) {
    ...
    if (use_xsdt) madt_phys = acpi_find_table_from_xsdt(root, "APIC");
    else madt_phys = acpi_find_table_from_rsdt(root, "APIC");
