		...
		for (dev.slot = 0; dev.slot < 32; dev.slot++) {
			for (dev.func = 0; dev.func < 8; dev.func++) {
				...
				uint32_t vendor_device =
					pci_config_read32(dev.bus, dev.slot, dev.func, 0x00);
				dev.identification.vendor = vendor_device & 0xFFFF;
				if (dev.identification.vendor == 0xFFFF)
					continue; // no device

				dev.identification.device = (vendor_device >> 16) & 0xFFFF;
				uint32_t classcode =
					pci_config_read32(dev.bus, dev.slot, dev.func, 0x08);
				...
				for (int bar = 0; bar < 6; bar++) {
					...
					dev.bar[bar] = pci_config_read32(dev.bus, dev.slot,
					                                 dev.func, 0x10 + bar * 4) &
					               ~0xF;
				}
...
static void pci_device_create_cb(struct pci_device *device) {
	...
	} else if (device->identification.vendor ==
	               intel_440fx_identification.vendor &&
	           device->identification.device ==
	               intel_440fx_identification.device) {
		_devices[_device_count].driver = &intel_440fx_driver;
	} else if (device->identification.vendor ==
	               intel_82371sb_isa_identification.vendor &&
	           device->identification.device ==
	               intel_82371sb_isa_identification.device) {
		_devices[_device_count].driver = &intel_82371sb_isa_driver;
	} else if (device->identification.vendor ==
	               intel_82371sb_ide_identification.vendor &&
	           device->identification.device ==
	               intel_82371sb_ide_identification.device) {
		_devices[_device_count].driver = &intel_82371sb_ide_driver;
	} else if (device->identification.vendor ==
	               intel_82371sb_acpi_identification.vendor &&
	           device->identification.device ==
	               intel_82371sb_acpi_identification.device) {
		_devices[_device_count].driver = &intel_82371sb_acpi_driver;
	} else if (device->identification.vendor ==
	               intel_82540em_identification.vendor &&
	           device->identification.device ==
	               intel_82540em_identification.device) {
		_devices[_device_count].driver = &intel_82540em_driver;
	} else if (device->identification.vendor ==
	               usb_xhci_identification.vendor &&
	           device->identification.device ==
	               usb_xhci_identification.device) {
		_devices[_device_count].driver = &usb_xhci_driver;
	} else if (device->identification.vendor ==
	               redhat_virtio_block_device_identification.vendor &&
	           device->identification.device ==
